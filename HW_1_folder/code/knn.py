...
class KNNClassifier:
    ...
    def fit(self, X, y):
        self.train_X = X
        self.train_y =np.asarray(y, dtype=int)
    ...
    def predict_labels_binary(self, distances):
        ...
        for i in range(n_test):
            ...
            k_nearest_labels = self.train_y[indices].astype(int)
            counts = np.bincount(k_nearest_labels)
            prediction[i] = counts.argmax()
    ...
    def predict_labels_multiclass(self, distances):
        ...
        n_test = distances.shape[0]
        prediction = np.zeros(n_test, dtype=int)  # Use dtype=int instead of np.int

        for i in range(n_test):
            indices = np.argsort(distances[i])[:self.k]
            k_nearest_labels = self.train_y[indices]
            counts = np.bincount(k_nearest_labels)
            prediction[i] = counts.argmax()
